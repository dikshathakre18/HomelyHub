...
class APIFeatures {
  ...
  search() {
    ...
    if (queryObj.guests) {
      searchQuery.maximumGuests = { $gte: queryObj.guests };
    }
    ...
    if (queryObj.dateIn && queryObj.dateOut) {
      const [inDay, inMonth, inYear] = queryObj.dateIn.split("-");
      const [outDay, outMonth, outYear] = queryObj.dateOut.split("-");

      const dateIn = new Date(`${inYear}-${inMonth}-${inDay}`);
      const dateOut = new Date(`${outYear}-${outMonth}-${outDay}`);
